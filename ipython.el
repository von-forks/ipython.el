...
(defadvice py-execute-region (around py-execute-buffer-ensure-process)
  ...
  (let ((py-which-shell (if (and (comint-check-proc "*Python*"))
			    py-python-command
...
in the current *Python* session."
      ...
      (let* ((ugly-return nil)
	     ...
	     (comint-preoutput-filter-functions
	      (append comint-preoutput-filter-functions
		      (if (memq color-filter comint-preoutput-filter-functions)
			  (list simplefn)
			(list color-filter simplefn)))))
	...
	(setq ugly-return
	      (replace-regexp-in-string ".*#PYTHON-MODE SILENT\n" "" ugly-return))
        (setq completions
